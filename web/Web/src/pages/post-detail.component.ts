      ...
      @if (post(); as p) {
        <div class="pb-20">
           ...
           @if (p.collaborators && p.collaborators.length > 0) {
             <div class="px-3 sm:px-4 mb-3 flex items-center gap-2 text-slate-500 text-sm">
               <app-icon name="users" [size]="16"></app-icon>
               <span>with </span>
               <span class="text-indigo-500 hover:underline cursor-pointer" (click)="navigateToProfile(p.collaborators[0].username)">
                 @{{ p.collaborators[0].username }}
               </span>
               @if (p.collaborators.length > 1) {
                 <span>and {{ p.collaborators.length - 1 }} {{ p.collaborators.length === 2 ? 'other' : 'others' }}</span>
               }
             </div>
           }
           ...
           @if (p.youtube_url) {
              <div class="mb-3 sm:mb-4">
                 <iframe 
                   [src]="getYouTubeEmbedUrl(p.youtube_url)" 
