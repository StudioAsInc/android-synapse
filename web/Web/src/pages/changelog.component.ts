            ...
            <div class="group" appAnimateOnScroll>
              ...
              <div class="rounded-xl p-6 transition-all border duration-300
                  ...
                  @for (change of entry.changes; track change.text) {
                    <li class="flex items-start gap-3">
                      ...
                      <span class="text-sm leading-relaxed text-slate-600 dark:text-slate-300">
                        ...
                        <span [innerHTML]="formatMarkdown(change.text)"></span>
                      </span>
                    </li>
                  }
                </ul>
                ...
                @if (entry.media && entry.media.length > 0) {
                  <div class="mt-4 grid gap-3" [class.grid-cols-2]="entry.media.length > 1">
                    @for (item of entry.media; track item.url) {
                      <div class="rounded-lg overflow-hidden border border-slate-200 dark:border-white/10">
                        @if (item.type === 'image') {
                          <img [src]="item.url" [alt]="item.caption || 'Screenshot'" class="w-full h-auto">
                        } @else {
                          <video [src]="item.url" controls class="w-full h-auto"></video>
                        }
                        @if (item.caption) {
                          <div class="px-3 py-2 text-xs text-slate-600 dark:text-slate-400 bg-slate-50 dark:bg-slate-950">
                            {{ item.caption }}
                          </div>
                        }
                      </div>
