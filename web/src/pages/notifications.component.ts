...
import { TagRequestService } from '../services/tag-request.service';
              ...
              <div class="flex gap-3">
                ...
                <div class="relative flex-shrink-0">
                  ...
                  <div 
                    ...
                    [class.bg-orange-500]="notification.type === 'tag_request'"
                    ...
                    @switch (notification.type) {
                      @case ('tag_request') {
                        <app-icon name="user-check" [size]="12" class="text-white"></app-icon>
                      }
                      @case ('LIKE') {
                ...
                <div class="flex-1 min-w-0">
                  ...
                  @if (notification.type === 'tag_request') {
                    <div class="mt-3 flex gap-2">
                      <button
                        (click)="acceptTagRequest(notification.id, $event)"
                        [disabled]="processingRequest() === notification.id"
                        class="flex-1 px-4 py-2 bg-green-500 hover:bg-green-600 disabled:opacity-50 text-white text-sm font-bold rounded-lg transition-colors flex items-center justify-center gap-2">
                        @if (processingRequest() === notification.id) {
                      ...
                      <button
                        (click)="rejectTagRequest(notification.id, $event)"
                        [disabled]="processingRequest() === notification.id"
                        class="flex-1 px-4 py-2 bg-slate-200 hover:bg-slate-300 dark:bg-slate-800 dark:hover:bg-slate-700 disabled:opacity-50 text-slate-900 dark:text-white text-sm font-bold rounded-lg transition-colors flex items-center justify-center gap-2">
...
export class NotificationsComponent implements OnInit {
  ...
  tagRequestService = inject(TagRequestService);
  ...
  filters = [
    { id: 'all', name: 'All' },
    { id: 'tag_requests', name: 'Tag Requests' },
    { id: 'likes', name: 'Likes' },
  ...
  ngOnInit() {
    ...
    // Load pending tag requests
    const userId = this.authService.currentUser()?.id;
    if (userId) {
      this.tagRequestService.loadPendingRequests(userId);
    }
  }
  ...
  filteredNotifications() {
    ...
    switch (this.activeFilter()) {
      case 'tag_requests':
        return all.filter(n => n.type === 'TAG_REQUEST');
      case 'likes':
  ...
  async acceptTagRequest(requestId: string, event: Event) {
    event.stopPropagation();
    this.processingRequest.set(requestId);
    const success = await this.tagRequestService.acceptTagRequest(requestId);
    if (success) {
  ...
  async rejectTagRequest(requestId: string, event: Event) {
    event.stopPropagation();
    this.processingRequest.set(requestId);
    const success = await this.tagRequestService.rejectTagRequest(requestId);
    if (success) {
      // Optionally show success message
    }
    this.processingRequest.set(null);
