...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, IconComponent, PostCardComponent, PostShimmerComponent, FormsModule],
  template: `
         ...
         <div class="px-4 flex justify-between items-end relative -mt-12 mb-4">
            ...
            <div class="mb-4 flex gap-2">
               @if (!isOwnProfile()) {
                 ...
                 <button (click)="sendMessage()" class="p-2 border border-slate-300 dark:border-white/20 rounded-full hover:bg-slate-50 dark:hover:bg-white/10">
...
export class ProfileComponent implements OnInit {
  ...
  async sendMessage() {
    ...
    try {
      const chatId = await this.messagingService.createChat([p.uid]);
      if (chatId) {
