...
export class SocialService {
    ...
    try {
      ...
      const postsWithLikes = await Promise.all(
        (data || []).map(async (post: any) => {
          ...
          const isReposted = userId ? await this.checkIfReposted(post.id, userId) : false;
          
          const { data: tagData } = await this.supabase
            .from('tag_requests')
            .select(`
              tagged_user_id,
              users:tagged_user_id (
                uid,
                username,
                display_name,
                avatar
          ...
          const collaborators = (tagData || []).map((tag: any) => tag.users).filter(Boolean);
  ...
  async checkIfReposted(postId: string, userId: string): Promise<boolean> {
    const { data } = await this.supabase
      .from('reposts')
      .select('id')
      .eq('post_id', postId)
