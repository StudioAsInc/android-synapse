...
// This activity is responsible for creating a user's profile after initial sign-up. It is heavily dependent on Firebase.
// 1. **Remove Firebase Dependencies**:
//    - Remove all `com.google.firebase.*` imports, including `FirebaseAuth`, `FirebaseDatabase`, `DatabaseReference`, and `FirebaseApp`.
//    - The `_firebase` and `auth` properties should be removed and replaced with a Supabase client instance.
//
// 2. **Refactor Username Validation**:
//    - The `TextWatcher` for the `username_input` field currently queries the Firebase Realtime Database to check for username availability.
//    - This logic needs to be replaced with a call to a Supabase function (e.g., `check_username_availability`) or a direct query to the `profiles` table to check if the username already exists.
//
// 3. **Refactor Profile Data Pushing**:
//    - The `complete_button`'s `OnClickListener` uses a `UserDataPusher` class to write the profile data to Firebase.
//    - The `UserDataPusher` class must be refactored to `insert` a new row into the `profiles` table in Supabase with the user's profile information.
//    - The authentication details (`email`, `uid`) should be sourced from the Supabase user object.
//
// 4. **Migrate Image Uploading**:
//    - The `onActivityResult` method uses `ImageUploader.uploadImage()` to upload the user's avatar.
//    - The `ImageUploader` class, which likely uses a service like ImgBB or Firebase Storage, needs to be migrated to use Supabase Storage.
//    - The `thedpurl` should be updated with the public URL of the image from Supabase Storage.
//
// 5. **Refactor Authentication Logic**:
//    - The activity includes numerous unused `OnCompleteListener` listeners for various Firebase Auth operations. These should be removed.
//    - The logic for handling account cancellation (`onBackPressed` and `onOptionsItemSelected`) uses `FirebaseAuth.getInstance().signOut()` and `FirebaseAuth.getInstance().getCurrentUser().delete()`.
//    - These calls must be replaced with the Supabase equivalents for signing out and deleting a user.
//
// 6. **Centralize OneSignal Player ID Logic**:
//    - The `addOneSignalPlayerIdToMap` method is responsible for adding the OneSignal Player ID to the user's data.
//    - This logic should be integrated into the new Supabase `UserDataPusher` or a similar service to ensure the Player ID is saved to the user's profile in the Supabase database.
					...
					pusher.pushData(
						...
						(success, errorMessage) -> {
							if (success) {
								...
								if (errorMessage.equals("Permission denied")) {
