...
import io.github.jan.supabase.gotrue.GoTrueException
...
class AuthActivity : AppCompatActivity() {
    ...
    private fun handleContinueClick(view: View) {
        ...
        nameFirstLetterTextView.text = username[0].toString().toUpperCase()
    ...
    private fun handleSignUpClick(view: View) {
        ...
        if (isValid) {
            lifecycleScope.launch {
                try {
                    Supabase.client.auth.signUpWith(Email(email, pass))
    ...
    private fun handleRegistrationError(exception: Exception?) {
        if (exception is GoTrueException && exception.message == "User already registered") {
    ...
    private fun handleExistingAccount() {
        ...
        lifecycleScope.launch {
            try {
                Supabase.client.auth.signInWith(Email(email, pass))
                fetchUsername(Supabase.client.auth.currentUserOrNull()?.id ?: "")
            } catch (e: Exception) {
                showSignInError()
            }
        }
    ...
    private fun fetchUsername(uid: String) {
        ...
        lifecycleScope.launch {
            try {
                val userProfile = Supabase.client.postgrest["profiles"].select {
                    filter("id", "eq", uid)
                }.data.let {
                    if (it.isNotEmpty()) {
                        it[0].decodeAs<UserProfile>()
                    } else {
                        null
                    }
                }
                ...
                if (userProfile != null) {
                    showWelcomeMessage("You are @${userProfile.username} right? No further steps, Let's go...")
                } else {
    ...
    private fun updateOneSignalPlayerId(uid: String) {
        if (OneSignal.getUser().pushSubscription.isOptedIn) {
