...
public class PostMoreBottomSheetDialog extends DialogFragment {
    ...
    public Dialog onCreateDialog(@Nullable Bundle savedInstanceState) {
        ...
        rootView = View.inflate(getContext(), R.layout.post_settings_cbsd, null);
    ...
    private void dialogStyles() {
        ...
        if (postPublisherUID.equals(authService.getCurrentUser().getUid()) || "mashikahamed0@gmail.com".equals(authService.getCurrentUser().getEmail())) {
    ...
    private void copyPostTextToClipboard() {
        if (postText != null && !postText.isEmpty()) {
            ClipboardManager clipboard = (ClipboardManager) getActivity().getSystemService(Context.CLIPBOARD_SERVICE);
            ClipData clip = ClipData.newPlainText("Post text", postText);
            clipboard.setPrimaryClip(clip);
            SketchwareUtil.showMessage(getActivity(), getResources().getString(R.string.text_copied_toast));
            dialog.dismiss();
        } else {
            SketchwareUtil.showMessage(getActivity(), getResources().getString(R.string.no_text_to_copy));
        }
    }
    
    private void sharePost() {
        String shareLink = "https://web-synapse.pages.dev/post.html?post=" + postKey;
        String shareText = postText != null && !postText.isEmpty() 
            ? postText + "\n\n" + shareLink 
            : shareLink;
        ...
        shareIntent.putExtra(Intent.EXTRA_SUBJECT, getResources().getString(R.string.share_post_subject));
        
        try {
            startActivity(Intent.createChooser(shareIntent, getResources().getString(R.string.share_post_chooser_title)));
        } catch (Exception e) {
            SketchwareUtil.showMessage(getActivity(), getResources().getString(R.string.share_error));
        }
    ...
    private void deletePostDialog(String key) {
		...
		dbService.getReference("posts/" + key).setValue(null, null);
		dbService.getReference("posts-comments/" + key).setValue(null, null);
		dbService.getReference("posts-comments-like/" + key).setValue(null, null);
		dbService.getReference("posts-likes/" + key).setValue(null, null);
	}
