...
public class ChatActivity extends AppCompatActivity {
	...
	private void initializeLogic() {
		...
		chatMessagesRef = _firebase.getReference("chats").child(chatID);
	...
	public void onDestroy() {
		...
		if (auth.getCurrentUser() != null) {
			// Typing indicator cleanup removed as per review feedback.
		}
	...
	private void _fetchRepliedMessages(ArrayList<HashMap<String, Object>> messages) {
		...
		String chatID = getChatId(myUid, theirUid);
		DatabaseReference chatRef = _firebase.getReference("chats").child(chatID);
	...
	public void _messageOverviewPopup(final View _view, final double _position, final ArrayList<HashMap<String, Object>> _data) {
		...
		editLayout.setOnClickListener(v -> {
			...
			dialog.setPositiveButton("Save", (d, w) -> {
				...
				DatabaseReference msgRef = _firebase.getReference("chats").child(chatID).child(msgKey);
	...
	public void _DeleteMessageDialog(final ArrayList<HashMap<String, Object>> _data, final double _position) {
		...
		zorry.setPositiveButton("Delete", new DialogInterface.OnClickListener() {
			...
			public void onClick(DialogInterface _dialog, int _which) {
				...
				_firebase.getReference("chats").child(chatID).child(messageKey).removeValue();
	...
	private void proceedWithMessageSending(String messageText, String senderUid, String recipientUid, String recipientOneSignalPlayerId) {
		...
		if (!attactmentmap.isEmpty()) {
			...
			if (allUploadsSuccessful && (!messageText.isEmpty() || !successfulAttachments.isEmpty())) {
				...
				_firebase.getReference("chats").child(chatID).child(uniqueMessageKey).setValue(ChatSendMap);
				_firebase.getReference("user-chats").child(senderUid).child(chatID).setValue(true);
				_firebase.getReference("user-chats").child(recipientUid).child(chatID).setValue(true);
	...
	public void _updateInbox(final String _lastMessage) {
		...
		ChatInboxSend.put(UID_KEY, otherUserUid);
		ChatInboxSend.put(LAST_MESSAGE_UID_KEY, currentUserUid);
		...
		ChatInboxSend.put("chatID", chatID);
		_firebase.getReference(SKYLINE_REF).child(INBOX_REF).child(currentUserUid).child(chatID).setValue(ChatInboxSend);

		// Update inbox for the other user
		ChatInboxSend2 = new HashMap<>();
		ChatInboxSend2.put(UID_KEY, currentUserUid);
		ChatInboxSend2.put(LAST_MESSAGE_UID_KEY, currentUserUid);
		...
		ChatInboxSend2.put("chatID", chatID);
		_firebase.getReference(SKYLINE_REF).child(INBOX_REF).child(otherUserUid).child(chatID).setValue(ChatInboxSend2);
	}
