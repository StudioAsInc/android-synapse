...
public class ChatActivity extends AppCompatActivity implements ChatAdapterListener {
	...
	private void initialize(Bundle _savedInstanceState) {
		...
		close_attachments_btn.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View _view) {
				...
				String chatId = ChatMessageManager.INSTANCE.getChatId(SupabaseManager.INSTANCE.getCurrentUserID(), getIntent().getStringExtra("uid"));
				drafts.edit().remove(chatId + "_attachments").apply();
				if (SupabaseManager.INSTANCE.getCurrentUserID() != null) {
					PresenceManager.setActivity(SupabaseManager.INSTANCE.getCurrentUserID(), "Idle");
				}
			}
		});
					...
					if (ReplyMessageID != null && !ReplyMessageID.equals("null")) {
						...
						if (repliedMessageIndex != -1) {
							...
							for (int i = startIndex; i <= endIndex; i++) {
								...
								String sender = message.get(UID_KEY).toString().equals(SupabaseManager.INSTANCE.getCurrentUserID()) ? "Me" : SecondUserName;
		...
		message_et.addTextChangedListener(new TextWatcher() {
			...
			public void onTextChanged(CharSequence _param1, int _param2, int _param3, int _param4) {
				...
				String chatID = ChatMessageManager.INSTANCE.getChatId(SupabaseManager.INSTANCE.getCurrentUserID(), getIntent().getStringExtra(UID_KEY));
				if (_charSeq.length() == 0) {
					...
					typingSnd.put(UID_KEY, SupabaseManager.INSTANCE.getCurrentUserID());
	...
	private void initializeLogic() {
		...
		// CRITICAL FIX: Set up Supabase reference to listen to the correct chat node
		// We need to listen to the chat node where messages are being sent/received
		String currentUserUid = SupabaseManager.INSTANCE.getCurrentUserID();
	...
	protected void onStart() {
		...
		if (SupabaseManager.INSTANCE.getCurrentUserID() != null) {
			...
			PresenceManager.setChattingWith(SupabaseManager.INSTANCE.getCurrentUserID(), recipientUid);
		}
	}
	...
	public void showMessageOverviewPopup(View _view, int _position, ArrayList<HashMap<String, Object>> _data) {
		...
		final boolean isMine = currentUserUid != null && senderUid != null && senderUid.equals(currentUserUid);
		...
		editLayout.setOnClickListener(v -> {
			...
			dialog.setPositiveButton("Save", (d, w) -> {
				...
				String myUid = SupabaseManager.INSTANCE.getCurrentUserID();
				if (myUid == null) {
					return;
				}
