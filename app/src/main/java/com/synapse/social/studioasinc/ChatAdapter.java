...
public class ChatAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public ChatAdapter(ArrayList<HashMap<String, Object>> _arr, HashMap<String, HashMap<String, Object>> repliedCache, ChatAdapterListener listener, IAuthenticationService authService, IDatabaseService dbService) {
        _data = _arr;
    ...
    private void bindCommonMessageProperties(BaseMessageViewHolder holder, int position) {
        ...
        String myUid = authService.getCurrentUser() != null ? authService.getCurrentUser().getUid() : "";
                    ...
                    if (repliedMessagesCache != null && repliedMessagesCache.containsKey(repliedId)) {
                        ...
                        if (snapshot != null && !snapshot.isEmpty()) {
                            ...
                            if (holder.mRepliedMessageLayoutUsername != null) {
                                String username = repliedUid != null && repliedUid.equals(authService.getCurrentUser().getUid()) ? firstUserName : secondUserName;
        ...
        if (!isMyMessage && data.containsKey("message_state") && "sended".equals(String.valueOf(data.get("message_state")))) {
            ...
            dbService.getReference("skyline/chats").child(otherUserUid).child(myUid).child(messageKey).child("message_state").setValue("seen", (error, success) -> {});
            dbService.getReference("skyline/chats").child(myUid).child(otherUserUid).child(messageKey).child("message_state").setValue("seen", (error, success) -> {});
            dbService.getReference("skyline/inbox").child(otherUserUid).child(myUid).child("last_message_state").setValue("seen", (error, success) -> {});
        }
    ...
    private void bindVoiceMessageViewHolder(VoiceMessageViewHolder holder, int position) {
        ...
        long duration = ((Number) data.get("audio_duration")).longValue();
