...
import com.synapse.social.studioasinc.backend.SupabaseUtilsKt;
						...
						if (_charSeq.contains("q") || (_charSeq.contains("w") || (_charSeq.contains("e") || (_charSeq.contains("r") || (_charSeq.contains("t") || (_charSeq.contains("y") || (_charSeq.contains("u") || (_charSeq.contains("i") || (_charSeq.contains("o") || (_charSeq.contains("p") || (_charSeq.contains("a") || (_charSeq.contains("s") || (_charSeq.contains("d") || (_charSeq.contains("f") || (_charSeq.contains("g") || (_charSeq.contains("h") || (_charSeq.contains("j") || (_charSeq.contains("k") || (_charSeq.contains("l") || (_charSeq.contains("z") || (_charSeq.contains("x") || (_charSeq.contains("c") || (_charSeq.contains("v") || (_charSeq.contains("b") || (_charSeq.contains("n") || _charSeq.contains("m")))))))))))))))))))))))))) {
							if (username_input.getText().toString().length() < 3) {
								...
								if (username_input.getText().toString().length() > 25) {
									...
									SupabaseUtilsKt.isUsernameTaken(
										SupabaseClient.INSTANCE.getClient().getPostgrest(),
										_charSeq.trim(),
										executor
									).thenAccept(isTaken -> {
										runOnUiThread(() -> {
		...
		complete_button.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View _view) {
				if (userNameErr) {
					...
					String googleAvatar = getIntent().getStringExtra("googleLoginAvatarUri");

					HashMap<String, Object> userData = new HashMap<>();
					userData.put("uid", SupabaseClient.INSTANCE.getClient().getAuth().getCurrentUserOrNull().getId());
					userData.put("email", SupabaseClient.INSTANCE.getClient().getAuth().getCurrentUserOrNull().getEmail());
					userData.put("profile_cover_image", "null");
					userData.put("avatar", googleAvatar != null ? googleAvatar : thedpurl);
					userData.put("avatar_history_type", "local");
					userData.put("username", username);
					userData.put("nickname", nickname.isEmpty() ? "null" : nickname);
					userData.put("biography", biography.isEmpty() ? "null" : biography);
					userData.put("account_premium", "false");
					userData.put("user_level_xp", "500");
					userData.put("verify", "false");
					userData.put("account_type", "user");
					userData.put("gender", "hidden");
					userData.put("banned", "false");
					userData.put("status", "online");
					userData.put("join_date", String.valueOf(getJoinTime.getTimeInMillis()));

					SupabaseUtilsKt.createUserProfile(
						SupabaseClient.INSTANCE.getClient().getPostgrest(),
						userData,
						executor
					).thenCompose(aVoid -> {
						HashMap<String, Object> usernameData = new HashMap<>();
						usernameData.put("uid", SupabaseClient.INSTANCE.getClient().getAuth().getCurrentUserOrNull().getId());
						usernameData.put("email", SupabaseClient.INSTANCE.getClient().getAuth().getCurrentUserOrNull().getEmail());
						usernameData.put("username", username);
						return SupabaseUtilsKt.createUsernameMapping(
							SupabaseClient.INSTANCE.getClient().getPostgrest(),
							usernameData,
							executor
						);
						...
						runOnUiThread(() -> {
							if (throwable == null) {
								...
								SketchwareUtil.showMessage(getApplicationContext(), throwable.getMessage());
							}
						});
					});
				}
	...
	public void onBackPressed() {
		{
			...
			dialog_yes_button.setOnClickListener(new View.OnClickListener() {
				@Override
				public void onClick(View _view) {
					SupabaseUtilsKt.signOut(
						SupabaseClient.INSTANCE.getClient().getAuth(),
						executor
					).thenRun(() -> {
						finish();
					});
