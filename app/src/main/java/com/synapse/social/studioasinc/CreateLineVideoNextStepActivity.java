...
public class CreateLineVideoNextStepActivity extends AppCompatActivity {
	...
	public void _LoadingDialog(final boolean _visibility) {
		...
		dbService.getReference("notify").push().updateChildren(m);
		m.clear();
		if (_visibility) {
			if (SynapseLoadingDialog== null){
				...
				SynapseLoadingDialog.getWindow().setBackgroundDrawable(new android.graphics.drawable.ColorDrawable(android.graphics.Color.TRANSPARENT));
	...
	public void _uploadLineVideo(final String _path, final boolean _isUrl) {
		if (_isUrl) {
			...
			dbService.getReference("skyline/line-posts").child(UniquePostKey).updateChildren(PostSendMap, new DatabaseReference.CompletionListener() {
				@Override
